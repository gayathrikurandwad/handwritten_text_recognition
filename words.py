...
def _text_detect(img, image, join=False):
    ...
    # Define dilation kernel for line or word segmentation
    kernel = np.ones((5, 100), np.uint16)  # Use (5, 30) for word segmentation
    ...
    # Loop through all contours at the top level
    while index >= 0:
        ...
        # Text detection filter criteria
        if (r > 0.1
                and 1600 > w > 10
                and 1600 > h > 10
                and h / w < 3
    ...
    # Join nearby bounding boxes if needed
    if join:
    ...
    for (x, y, w, h) in boxes:
        cv2.rectangle(small_rgb, (x, y), (x + w, y + h), (0, 255, 0), 2)
        bounding_boxes = np.vstack((bounding_boxes, np.array([x, y, x + w, y + h])))

    # Display the image with bounding boxes (ensure implt is defined)
    #implt(small_rgb, t='Bounding rectangles')

    # Adjust bounding boxes back to original image size
    boxes = bounding_boxes.dot(ratio(image, small.shape[0])).astype(np.int64)

    return boxes[1:]  # Return detected bounding boxes
